...
uint8_t BT_MC_generateStatusMsg(uint8_t state){
	char TLV_Buffer[12];
	  ...
	  if ((state == BT_IDLE) || (state == BT_FINISH)){
		  ...
		  generateTLV_I(TLV_Buffer,CARDING_DUCT_STATE,C.D.autoFeed_ductState_toApp);
		  add_TLVBuf_To_TxBuf(TLV_Buffer,TLV_INT,initLength+tlvSize);
		  tlvSize += TLV_INT;

		  generateTLV_I(TLV_Buffer,RUN_COILER_SENSOR,C.D.cardFeed_ductState_toApp);
		  add_TLVBuf_To_TxBuf(TLV_Buffer,TLV_INT,initLength+tlvSize);
		  tlvSize += TLV_INT;

		  if (S.BT_runInfoToSend == RUN_PRODUCTION_DATA){
			  generateTLV_F(TLV_Buffer,RUN_OUTPUT_MTRS_PER_SPINDLE,C.L.currentMtrsRun);
			  add_TLVBuf_To_TxBuf(TLV_Buffer,TLV_FLOAT,initLength+tlvSize);
			  tlvSize += TLV_FLOAT;

			  generateTLV_F(TLV_Buffer,RUN_TOTAL_POWER,C.L.mcPower); // totalPower
			  add_TLVBuf_To_TxBuf(TLV_Buffer,TLV_FLOAT,initLength+tlvSize);
			  tlvSize += TLV_FLOAT;
		  }else{
			  //send motor Data, if not carding or beater
			  if (S.BT_runInfowhichMotor >= 2){
				  ...
				  generateTLV_I(TLV_Buffer,RUN_TLV_MOTOR_TEMP, ER[S.BT_runInfowhichMotor].MOTTemp);
				  add_TLVBuf_To_TxBuf(TLV_Buffer,TLV_INT,initLength+tlvSize);
				  tlvSize += TLV_INT;

				  generateTLV_I(TLV_Buffer,RUN_TLV_MOSFET_TEMP,ER[S.BT_runInfowhichMotor].FETTemp);
				  add_TLVBuf_To_TxBuf(TLV_Buffer,TLV_INT,initLength+tlvSize);
				  tlvSize += TLV_INT;

				  generateTLV_F(TLV_Buffer,RUN_TLV_MOTOR_CURRENT,ER[S.BT_runInfowhichMotor].peakPhaseCurrentApk);
				  add_TLVBuf_To_TxBuf(TLV_Buffer,TLV_FLOAT,initLength+tlvSize);
				  tlvSize += TLV_FLOAT;

				  generateTLV_I(TLV_Buffer,RUN_TLV_MOTOR_RPM,(uint16_t)abs(ER[S.BT_runInfowhichMotor].actualRPM));
				  add_TLVBuf_To_TxBuf(TLV_Buffer,TLV_INT,initLength+tlvSize);
				  tlvSize += TLV_INT;
